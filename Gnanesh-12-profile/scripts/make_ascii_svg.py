...
GAMMA = float(os.environ.get("GAMMA", 0.75))          # NOTE: on this 0-1 luminance scale, gamma<1 brightens mids (pow(x,g) with g<1 raises x); tuned empirically for this photo
SHARPEN = False
WHITE_FLOOR = float(os.environ.get("WHITE_FLOOR", 0.85))    # luminance above this is forced to blank (space)
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'avi@github:~$ whoami <tspan fill="{INK}">Gnanesh</tspan></text>')
